...
#undef min
	...
	BOOL CALLBACK QuickEnumWindowsProc(HWND hwnd, LONG lParam) {
	...
	#endif
		wchar_t windowText[MAX_PATH] = { 0x00 };
		GetWindowText(hwnd, windowText, MAX_PATH - 1);
		if (GetLastError() != 0) {
			/*
			char msg[0x100] = { 0x00 };
		...
		size_t currentWindowTextLength = wcslen(windowText);
		if (currentWindowTextLength < 2)
			return TRUE;
	
		if (windowExeName != nullptr) {
			std::wstring exeName = L"";
			getHandleName(hwnd, &exeName);
			if (exeName.find(windowExeName) == -1) {
	#ifdef _MY_DEBUG
		...
		if (lParam < WINDOW_THREAD_SEARCH) {
			...
			if (lParam & WINDOW_SHARP_SEARCH) {
				if (_wcsicmp(windowText, windowName) == 0) {
					found = true;
				}
			}
			if (lParam & WINDOW_FUZZY_SEARCH) {
				...
				for (unsigned int i = 0; i<currentWindowTextLength && !found; i++) {
					if (windowText[i] == windowName[0]) {
						if (i + wantedWindowTextLength > currentWindowTextLength)
							break;

						for (unsigned int j = 0; j < wantedWindowTextLength; j++) {
							if (windowText[i + j] != windowName[j]) {
								found = false;
								break;
							}
							found = true;
	...
	#endif
			if (realId == (lParam % WINDOW_THREAD_SEARCH) || otherThreadId == (lParam % WINDOW_THREAD_SEARCH)) {
				hWindow = hwnd;
				return false;
			}
		}
	...
	void getFilesFromDirectory(std::string& filePath, std::vector<std::string>& result, bool parseSubDirectories) {
		return getFilesFromDirectory(filePath, std::string(""), result, parseSubDirectories);
	}
	void getFilesFromDirectory(std::string& filePath, std::string& extensionFilter, std::vector<std::string>& result, bool parseSubDirectories) {
		...
		do {
			...
			if (dataAttrib.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
				if(parseSubDirectories) {
					...
					getFilesFromDirectory( fileName, extensionFilter, result, parseSubDirectories);
				}
			}
			else {
	...
	bool ejectDLL(DWORD threadId, std::wstring dllName) {
		HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, false, threadId);
		if (!hProcess)
			throw TraceableExceptionARGS("OpenProcess failed! ErrorCode: %i", GetLastError());
	...
	DWORD fnumlen(double number) {
		...
		double checkNum = 0;
		DWORD len = 0x00; DWORD maxNill = 3;
		...
		while (true) {
			...
			if (checkNum >= 1000.00f && ((DWORD)checkNum % 1000 == 999 || (DWORD)checkNum % 1000 == 0)) {
				break;
			}
			len++;
		}
