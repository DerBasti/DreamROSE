...
//----------------------------------------------------------------------------
unsigned ctGenerateNextValue(CryptTable *CryptTables, CryptTableControlStruct *ControlStruct)
{
...
//-------------------------------------------------------------------------
bool ctValueExistsInTable(CryptTable *CryptTables, unsigned char Table, unsigned Value, unsigned short EntryCount)
{
...
void GenerateCryptTables(CryptTable* &CryptTables, unsigned ModValue)
{
	...
	CryptTables = (CryptTable*)malloc(sizeof(CryptTable));
	...
	CryptTables->startValue = ModValue;
	...
	CryptTables->additionalTables = (unsigned short*)calloc(0x200, sizeof(unsigned short));
	...
	do
	{
		...
		if ((unsigned)CryptTables->additionalTables[ivalue] == value) { value = 0xFFFFFFFF; break; }

		// If value exist, continue
		if (value == 0xFFFFFFFF) continue;

		// Save value
		CryptTables->additionalTables[entry++] = value;
	} while (entry<0x200);
}
...
void GenerateLoginServerCryptTables(CryptTable* &CryptTables)
{
	...
	// Another startvalue
	CryptTables->startValue = LOGINSERVER_CRYPTTABLE_START_VALUE;
...
void FreeCryptTables(CryptTable* &CryptTables)
{
	...
	// Free memory
	free(CryptTables->additionalTables);
...
int DecryptBufferHeader(CryptStatus *ri, CryptTable *CryptTables, unsigned char *Buffer)
{
	...
	*((unsigned*)Buffer) = *((unsigned     *)&TempBuffer);
	...
	ri->currentValue = (ri->currentValue * 0x343FD) + 0x269EC3;
